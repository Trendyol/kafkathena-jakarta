...
@DependsOnKafkathena
@Slf4j
public class FailoverErrorTopicTestableConsumer extends TestableConsumer {

    @KafkaListener(
            topics = "${kafkathena.consumers[json-consumer-failover-error-topic].topic}",
            groupId = "${kafkathena.consumers[json-consumer-failover-error-topic].props[group.id]}",
            containerFactory = "${kafkathena.consumers[json-consumer-failover-error-topic].factory-bean-name}"
    )
